...
func TestDriver_EndToEnd_ProvisionArmsGradeAggregate(t *testing.T) {
	...
	// PATH wiring: WITH resolves donmai; WITHOUT does not (the contamination guard).
	var withEnv, withoutEnv []string
	...
	if _, ok := BinaryOnPath("donmai", envPath(withEnv)); !ok {
		t.Error("WITH arm env must resolve donmai")
	}
	if _, ok := BinaryOnPath("donmai", envPath(withoutEnv)); ok {
