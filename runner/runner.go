...
func (r *Runner) Run(ctx context.Context, qw QueuedWork) (*Result, error) {
	...
	if leasePrepared {
		postOutcome = r.poster.PostPreparedOutcome(runCtx, qw.SessionID, res.Result, preparedStatusBody)
	} else {
		postOutcome = r.poster.PostWithOptionsOutcome(runCtx, qw.SessionID, res.Result, result.PostOptions{})
	}
