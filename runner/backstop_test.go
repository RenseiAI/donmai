...
func TestLookupGitHubPullRequestUsesSupportedGhFields(t *testing.T) {
	worktree := t.TempDir()
	gitInit(t, worktree)
	stubGhOnPathRejectingJSONField(t, "repository", 0, `{"number":463,"url":"https://github.com/RenseiAI/donmai/pull/463","baseRefName":"main","headRefName":"worktree-pr-fact-github-origin-guard"}`)
	fact := lookupGitHubPullRequest(context.Background(), worktree, "https://github.com/RenseiAI/donmai/pull/463", map[string]struct{}{
		"renseiai/donmai": {},
	})
	if fact == nil {
...
func stubGhOnPathRejectingJSONField(t *testing.T, rejectedField string, exitCode int, output string) {
	t.Helper()
	dir := t.TempDir()
	outFile := filepath.Join(dir, "gh-output.txt")
	if err := os.WriteFile(outFile, []byte(output), 0o600); err != nil {
...
case "$*" in
  *--json*%[1]s*)
    echo 'unknown JSON field: "%[1]s"' 1>&2
    exit 1
    ;;
esac
cat %[2]q 1>&2
exit %[3]d
`, rejectedField, outFile, exitCode)
	ghPath := filepath.Join(dir, "gh")
	//nolint:gosec // G306: a stub executable must carry the exec bit.
	if err := os.WriteFile(ghPath, []byte(script), 0o700); err != nil {
