...
func TestInteractive_CapabilityFailure(t *testing.T) {
	...
	qw := QueuedWork{}
	qw.SessionID = "s"
	out, err := r.dispatchInteractive(context.Background(), h, t.TempDir(), qw, res, noopSink{}, nil, nil)
	if err == nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out, err := minimalRunner(t).dispatchInteractive(
				context.Background(), handle, t.TempDir(), qw, res, sink, nil, nil,
			)
			if err != nil {
...
func TestInteractive_LocalOnlyCompletes(t *testing.T) {
	...
	defer cancel()
	out, err := r.dispatchInteractive(ctx, h, t.TempDir(), qw, res, noopSink{}, nil, nil)
	if err != nil {
...
func TestInteractive_LocalOnlyNonzeroExitFails(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	out, _ := r.dispatchInteractive(ctx, h, t.TempDir(), qw, res, noopSink{}, nil, nil)
