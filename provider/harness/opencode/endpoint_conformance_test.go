...
func TestEndpointBoundLaneA_LMStudioConformance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			p := &Provider{binary: binary, configTempDir: configRoot}
			...
			case <-ctx.Done():
				stderr := ""
				if cli, ok := h.(*openCodeHandle); ok {
					stderr = cli.stderrBuf.String()
				}
				t.Fatalf("LM Studio-shaped request not observed: %v (child stderr: %s)", ctx.Err(), stderr)
			}
...
func TestEndpointBoundCLIHelperProcess(t *testing.T) {
	...
	if err != nil {
		fail("resolve helper root: %v", err)
	}
