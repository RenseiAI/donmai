...
func TestPromptAdaptation_ExplicitDowngradeToOpenCodePrompt(t *testing.T) {
	t.Parallel()
	profile, _ := (&Provider{}).Manifest().PromptProfile(agent.PromptModeAutonomous)
	plan := openCodePromptPlan(true)
	adapted, receipt, err := agent.AdaptPrompt(agent.Spec{PromptPlan: &plan}, profile)
	if err != nil {
		t.Fatal(err)
	}
	if got, want := adapted.Prompt, "protocol\n\nrole\n\ncontext\n\nuser\n\namend"; got != want {
		t.Fatalf("prompt wire = %q, want %q", got, want)
	}
	if got, want := buildOpenCodeArgs(adapted), []string{"run", "--format", "json"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("argv = %#v, want %#v", got, want)
	}
	if receipt.ProfileID != "opencode/headless/prompt-v1" {
...
func openCodePromptPlan(authorize bool) agent.PromptPlan {
