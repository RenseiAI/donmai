...
const (
	...
	// maxAbsenceDischargesPerPass bounds how many lineages ONE pass discharges.
	//
	// A discharge is not cheap and it is serial and inline: two separated
	// readings with a 250 ms sleep between them, two dials, several registry
	// scans, and two or three control-plane round trips. The callers are the
	// surfaces that must stay responsive — every occupancy and heartbeat
	// surface drives the sweep, and the startup pass runs before the first
	// batch is composed — so a host coming back from a crash with twenty
	// vanished lineages would otherwise add ten seconds and forty round trips
	// to a single admission check, or to boot.
	//
	// Nothing is lost by stopping: what this pass does not discharge stays in
	// the quarantine projection (or, at boot, in the stale set, where it is
	// DECLARED rather than dropped), and the next pass picks it up. The bound
	// is on latency per pass, never on how many lineages can be discharged.
	maxAbsenceDischargesPerPass = 4
)
...
func (d *Daemon) proveSessionShimAbsence(
	...
	sidecar, held, sidecarErr := registry.GetWithdrawnAbsence(key.identity, key.shimID, key.processEpoch)
	if sidecarErr != nil {
		// A sidecar that cannot be read strands its lineage: the discovery
		// record is already gone, so the fall-through below finds nothing and
		// this incarnation can never be discharged again. Nothing here can fix
		// that, but going silent means nothing can FIND it either — the one
		// artifact naming the lineage is unreadable and no line says so.
		slog.Warn("session shim: withdrawn-record sidecar is unreadable; this lineage cannot be discharged until it is repaired",
			"session", key.identity.String(), "shim", key.shimID,
			"processEpoch", key.processEpoch, "error", sidecarErr)
		return SessionShimAbsentAttestation{}, false
	}
	if held {
	...
	if !d.sessionShimAbsenceStillHoldsAtWithdrawal(registry, key, record) {
		return SessionShimAbsentAttestation{}, false
	}
...
func (d *Daemon) proveWithdrawnSessionShimAbsence(
	...
	if !d.sessionShimAbsenceStillHoldsAtWithdrawal(registry, key, sidecar) {
		return SessionShimAbsentAttestation{}, false
	}
...
// sessionShimAbsenceStillHoldsAtWithdrawal re-checks, immediately before the
// step that cannot be undone, the two facts that can change while the probe is
// running.
//
// BOTH readings are last-reads, and for the same reason. A shim publishes its
// tombstone before it spends its courtesy waits, so a process that reads as
// gone has already written one if its finalizer ran at all — which makes the
// late tombstone read the load-bearing one and the read at the top of the probe
// merely an optimisation.
//
// The dial needs it just as much, and that took a round to see. The probe's
// first dial happens BEFORE the separation, so at least 250 ms and four steps
// separate it from the unlink — long enough for a shim to bind its listener and
// still be attested absent. Worse, the population this runs over is SELECTED
// for having an unreachable socket: the sweep walks the quarantine set, whose
// dominant reason is `socket_unreachable`, so "nothing answered" is close to
// tautological on exactly the rows where the second opinion is doing the most
// work. A stale dial there leaves the discharge resting on two process-table
// readings, which is what this file's header says it must never do.
//
// One connect, and a live listener vetoes the whole attestation.
func (d *Daemon) sessionShimAbsenceStillHoldsAtWithdrawal(
	registry *sessionshim.Registry,
	key shimIncarnation,
	record sessionshim.Record,
) bool {
	...
	if sessionShimSocketAnswers(record.SocketPath) {
		slog.Info("session shim: the socket began answering before the withdrawal; retaining the lineage",
			"session", key.identity.String(), "shim", key.shimID)
		return false
	}
	return true
...
func (d *Daemon) attestAbsentStaleSessionShims(
	...
	for _, rec := range stale {
		if discharged >= maxAbsenceDischargesPerPass {
			// Boot latency is bounded, not the work. What is left here is
			// DECLARED by the caller and swept on a later pass.
		...
		accepted, _ := d.attestAbsentSessionShim(ctx, registry, key, absenceOriginStartupAdoption)
		if !accepted {
			remaining = append(remaining, rec)
			continue
		}
...
func (d *Daemon) resubmitWithdrawnSessionShimAbsences(ctx context.Context, registry *sessionshim.Registry) int {
	...
	for _, rec := range withdrawn {
		if resubmitted >= maxAbsenceDischargesPerPass {
			// Same bound, same reason: the sidecars this pass does not reach
			// are still on disk and the next boot or sweep re-submits them.
			break
		}
		key := shimIncarnation{
			identity: rec.Identity(), shimID: rec.ShimID, processEpoch: rec.ProcessEpoch,
