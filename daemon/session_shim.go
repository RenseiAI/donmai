...
func (d *Daemon) AcknowledgeSessionShimRecoveryHeartbeat(
	...
	if d.shims.dynamicPublicationFailed &&
		(!d.shims.activationFailureRecoveryArmed || len(d.shims.failedActivations) != 0) {
		d.shims.mu.Unlock()
		return
	}
	...
	if d.shims.dynamicPublicationFailed {
		// Re-check at the final reopening edge. A consumer can record a failed
		// activation after this acknowledgement's first snapshot, and the inner
		// publication latch must remain closed unless the exact recovered set is
		// still empty after every heartbeat and lifecycle check passed.
		d.shims.mu.Lock()
...
// operatorOrphanDeadline reads the §D8 orphan deadline an operator set on this
// installed host, in milliseconds.
//
// The knob already exists on the launch contract this daemon writes into every
// shim's environment; reading the same key here is what makes it settable from
// outside the embedder's source, which is where an operator actually is when a
// deployment's grace turns out to be wrong. An embedder that configures
// OrphanPolicy.Deadline explicitly still wins — this only fills the default —
// and an unreadable or non-positive value is ignored rather than obeyed, since
// OrphanPolicy.Validate would refuse a daemon that took it literally.
func operatorOrphanDeadline() (time.Duration, bool) {
	raw, ok := os.LookupEnv(sessionshim.EnvOrphanDeadlineMS)
	if !ok || raw == "" {
		return 0, false
	}
	millis, err := strconv.ParseInt(raw, 10, 64)
	if err != nil || millis <= 0 {
		slog.Warn("session shim: ignoring an unreadable operator orphan deadline",
			"env", sessionshim.EnvOrphanDeadlineMS, "value", raw)
		return 0, false
	}
...
func (d *Daemon) adoptSessionShims(ctx context.Context) error {
	...
	staleBeforeDischarge := len(result.Stale)
	result.Stale = d.attestAbsentStaleSessionShims(ctx, registry, result.Stale)
	attestedAbsent := staleBeforeDischarge - len(result.Stale)
	...
	resubmittedAbsent := d.resubmitWithdrawnSessionShimAbsences(ctx, registry)
	...
	if cfg.OnAdoptionBatch != nil {
		...
		for _, quarantined := range result.Quarantined {
			if !sessionShimQuarantineNamesALineage(quarantined) {
				...
				// no real scope there — and the two identity fields are
				// both-or-neither in every producer: the undecodable-entry path
				// sets neither, and every other quarantine takes its identity
				// from a record whose decode validated both. A future producer
				// that set an organization WITHOUT a session id would break
				// that, and would need the second guard back.
				continue
			}
		...
		for _, orgID := range scopeOrgIDs {
			...
			if batchErr != nil {
				// A boot batch the control plane refuses because it ALREADY
				// HOLDS this lineage's adoption evidence — the planned-restart
				// collision — must cost that lineage its controller authority
				// and nothing else. Aborting here would close every controller
				// on the host and bring durable sessions up OFF, which is how a
				// single lineage's bookkeeping conflict reaped healthy
				// harnesses on an installed host.
				receipt, batchErr = d.commitBootBatchAroundRecordedEvidence(
	...
	slog.Info("session shim: startup adoption complete",
		...
		// Without these two, staleDeclared simply drops from N to N-discharged
		// with nothing on the line naming the difference — and "why is this
		// host declaring fewer lineages than it found?" is the first question
		// asked of a boot that discharged some.
		"attestedAbsent", attestedAbsent,
		"resubmittedAbsent", resubmittedAbsent,
...
func (d *Daemon) completeSessionShimAdoptionBatch(ctx context.Context, batch SessionShimAdoptionBatch) (SessionShimAdoptionBatchReceipt, error) {
	...
	if cfg.PrepareAdoptionBatch != nil {
		...
		cancel()
		if err != nil {
			// A preparation that reports the expected revision already advanced
			// past ours, by exactly one, for a batch whose digest is the one we
...
// admitted. Both routes in — the ordinary commit answer and the receipt an
// advanced-revision preparation echoed back for an already-committed batch —
// pass through it, so an adopted receipt is held to exactly the completeness
// the committed one is.
func (d *Daemon) validateSessionShimAdoptionBatchReceipt(
	batch SessionShimAdoptionBatch,
	...
	if (d.sessionShimEnabled() || d.sessionShimConfig().OnAdoptionPublished != nil) && receipt.AdoptionRevision == "" {
...
func (d *Daemon) republishSessionShimProjection(ctx context.Context, orgID string) error {
	...
	if err != nil {
		...
		if errors.As(err, &advanced) {
			// The control plane moved and this batch is NOT the batch that moved
			// it (an adoptable advance never reaches here — it returns a
			// receipt). Re-present the complete current projection at the
			// advanced revision, which is what one reconciliation pass does
			// after the refresher relearns the revision.
			d.scheduleSessionShimReconciliation(orgID, sessionShimReconcileCauseRevisionAdvanced)
		} else if errors.Is(err, errSessionShimAmbiguousBatchCommit) {
			...
			d.scheduleSessionShimReconciliation(orgID, sessionShimReconcileCauseAmbiguous)
		}
	...
	if revisionErr := d.retainRepublishedSessionShimReceipt(orgID, batch, receipt); revisionErr != nil {
...
func (d *Daemon) retainRepublishedSessionShimReceipt(
	scope string,
	batch SessionShimAdoptionBatch,
	receipt SessionShimAdoptionBatchReceipt,
) error {
	...
	// Resolve only from terminal evidence in the exact batch that this receipt
	// confirms. Live tombstoned state can advance while the callback is in
	// flight; consulting it here would let an older receipt authorize a newer
	// terminal disposition it never carried.
	for failed := range d.shims.failedActivations {
		if failed.identity.OrgID != scope {
			continue
		}
		for _, evidence := range batch.Tombstoned {
			terminal := evidence.Tombstone
			if terminal.GroupReaped && terminal.Identity() == failed.identity && terminal.ShimID == failed.shimID &&
