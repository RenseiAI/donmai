...
func sessionShimBatchDeclaringOmittedLineages(
	...
	for _, lineage := range omitted {
		...
		if lineage.Identity.SessionID == "" || lineage.ShimID == "" || lineage.ProcessEpoch == 0 {
			...
			slog.Warn("session shim: a reported omitted lineage names no exact incarnation and cannot be declared; "+
				"nothing local repairs this one (shim-omitted-lineage-kind-2026-09-06)",
				"session", lineage.Identity.String(), "arm", string(lineage.Kind),
				"shim", lineage.ShimID, "processEpoch", lineage.ProcessEpoch)
			continue
		}
		if !lineage.Kind.Declarable() {
			...
			//
			// This line is what makes "visibly" true of the CAUSE and not only
			// of the outcome. A stand-down looks identical from the status
			// surface whichever skip produced it, and the three have quite
			// different responses — an unreported arm is repaired by upgrading
			// the composer and the host recovers on the next install, while the
			// other two are facts about the control plane that nothing local
			// changes.
			slog.Warn("session shim: a reported omitted lineage carries no arm this daemon recognises, so the "+
				"generation rule for it cannot be chosen and it is left undeclared — upgrade the composing binary "+
				"to report the arm (shim-omitted-lineage-kind-2026-09-06)",
				"session", lineage.Identity.String(), "arm", string(lineage.Kind))
			continue
		}
		if lineage.Kind == SessionShimOmittedLineageAdopted && lineage.ControllerGeneration == 0 {
			...
			slog.Warn("session shim: the control plane reported an adopted lineage without a controller generation, "+
				"so it cannot be declared at a generation its exact-key lookup would match; nothing local repairs "+
				"this one (shim-omitted-lineage-kind-2026-09-06)",
				"session", lineage.Identity.String(), "arm", string(lineage.Kind),
				"shim", lineage.ShimID, "processEpoch", lineage.ProcessEpoch)
			continue
		}
