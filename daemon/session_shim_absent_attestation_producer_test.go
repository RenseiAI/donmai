...
func (f *absentProducerFixture) liveSocket() string {
	return f.bindSocket(filepath.Join(f.dir, "live.sock"))
}

// bindSocket starts a listener on an exact path. The absence probe's dials are
// the only thing that reads it, and they close immediately, so accepting and
// closing is a faithful stand-in for a shim that is answering.
func (f *absentProducerFixture) bindSocket(path string) string {
	f.t.Helper()
...
func TestSweepAttestsOnlyAnUnobservableLineage(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantAbsent {
				// THE REPUBLISH, behaviourally. The receiver prunes its own
				// quarantine snapshot for ordinary terminal evidence but NOT
				// for an absent attestation on this seam, so this publish is
				// the only thing that reconciles the two sets — without it the
				// next beat presents a set the last committed batch disagrees
				// with, is refused revision-stale, and drains the host on every
				// beat. An AST scan proves the identifier is present; it cannot
				// see the call wrapped in `if false {`, which is the shape this
				// assertion catches.
				f.mu.Lock()
				published := append([]SessionShimAdoptionBatch(nil), f.batches...)
				f.mu.Unlock()
				if len(published) == 0 {
					t.Fatal("the discharge published no projection; the receiver never learns the lineage left the " +
						"quarantine set and refuses every later beat as revision-stale")
				}
				last := published[len(published)-1]
				if last.OrgID != id.OrgID {
					t.Errorf("the projection was published for %q, want the discharged lineage's scope %q", last.OrgID, id.OrgID)
				}
				for _, q := range last.Quarantined {
					if q.Identity() == id && q.ShimID == shimID && q.ProcessEpoch == epoch {
						t.Errorf("the republished projection still names the discharged lineage: %+v", q)
					}
				}
...
func TestAbsenceProbeTreatsEveryUncertainReadingAsRetain(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			// The socket the FIRST dial found dead begins answering during the
			// separation. A dial taken only before the window is the stalest
			// reading in the chain — 250 ms and four steps from the unlink —
			// and it is the sole discriminator for the case it was added for,
			// on a population selected for having an unreachable socket. So it
			// is re-taken immediately before the withdrawal, beside the second
			// tombstone read, and a listener that came back vetoes the whole
			// attestation.
			name: "the socket begins answering before the withdrawal",
			mutate: func(f *absentProducerFixture, _ sessionshim.Identity, _ string, _ uint64) {
				f.bindSocket(f.deadSocketPath())
			},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
func TestAbsentAttestationRefusalThrottlesTheProbe(t *testing.T) {
	...
	// COUNT THE PROBE, not the cool-down. Asserting the mark exists and its
	// retryAt is armed looks like it pins this, and does not: the deferred
	// release CREATES the reportingTerminal entry and arms retryAt from a zero
	// value, so those assertions hold whether or not the claim ever gated the
	// probe. Moving only the claim below the probe — leaving the defer at the
	// top — is a faithful reproduction of the defect and passes all of them.
	// The property is that a second pass does no probe work, and the only way
	// to see that is to count the work.
	var reads atomic.Int64
	f.daemon.shims.mu.Lock()
	f.daemon.shims.absenceLiveness = func(identity sessionshim.ProcessIdentity) (bool, error) {
		reads.Add(1)
	...
	f.daemon.reconcileQuarantinedTombstones()
	afterSecond := reads.Load()

	if afterFirst == 0 {
		t.Fatal("the first pass never probed at all; this test is not exercising the probe it claims to bound")
	}
	if afterSecond != afterFirst {
		t.Fatalf("liveness reads after pass1=%d after pass2=%d — the second pass probed before consulting the claim, "+
			"so every occupancy and heartbeat surface pays a tombstone read, a full registry scan and a syscall per "+
			"quarantined lineage", afterFirst, afterSecond)
	}
