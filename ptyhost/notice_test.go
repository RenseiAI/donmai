...
func spawnRawCat(t *testing.T) (*Session, agent.InteractiveSubscription) {
	t.Helper()
	s := mustSpawn(t, Spec{Command: rawCatCommand(), NoticeDelivery: agent.NoticeDeliveryPTYNotice})
...
func TestSession_TryWriteNoticeAfterExit(t *testing.T) {
	s := mustSpawn(t, Spec{Command: []string{"/bin/sh", "-c", "exit 0"}, NoticeDelivery: agent.NoticeDeliveryPTYNotice})
...
// TestSession_TryWriteNoticeRefusedUnlessTheHarnessDeclaredPTYNotice is the
// structural gate, at the layer that owns the bytes.
//
// Everything else in this file tests WHEN a notice is safe on a terminal that
// is allowed to take one. This tests WHETHER it is allowed at all — and the
// answer comes from the harness's declaration, carried on Spec, not from
// anything the terminal can observe. That asymmetry is the whole point: the
// dangerous case (an inline modal in a live agent UI, where the submit byte
// picks the highlighted option) is INVISIBLE at this layer, so permission
// cannot be inferred here and must arrive declared.
//
// The refusal wraps agent.ErrUnsupported rather than returning the transient
// (false, nil): it is fixed for the life of the session, so a caller must be
// able to tell "never" from "not right now".
func TestSession_TryWriteNoticeRefusedUnlessTheHarnessDeclaredPTYNotice(t *testing.T) {
	tests := []struct {
		name      string
		declared  agent.NoticeDelivery
		wantWrite bool
	}{
		{name: "declared pty-notice", declared: agent.NoticeDeliveryPTYNotice, wantWrite: true},
		{name: "declared hook", declared: agent.NoticeDeliveryHook},
		{name: "declared mcp-rpc", declared: agent.NoticeDeliveryMCPRPC},
		{name: "declared none", declared: agent.NoticeDeliveryNone},
		{name: "undeclared", declared: ""},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			s := mustSpawn(t, Spec{Command: rawCatCommand(), NoticeDelivery: tc.declared})
			sub := subscribeOutput(t, s)
			waitForBytes(t, sub, ptyReadyMarker, 10*time.Second)

			written, err := s.TryWriteNotice([]byte("NOTICE-GATED\r"))
			if written != tc.wantWrite {
				t.Fatalf("TryWriteNotice written=%v, want %v", written, tc.wantWrite)
			}
			if tc.wantWrite {
				if err != nil {
					t.Fatalf("TryWriteNotice on a pty-notice session: %v", err)
				}
				return
			}
			if !errors.Is(err, agent.ErrUnsupported) {
				t.Fatalf("refusal error = %v; want one wrapping agent.ErrUnsupported", err)
			}
			// And nothing left the process: a refusal that still wrote would
			// be the exact defect, dressed as a denial.
			if _, err := s.WriteInput([]byte("probe\r")); err != nil {
				t.Fatalf("WriteInput: %v", err)
			}
			seen := waitForBytes(t, sub, "probe\r", 10*time.Second)
			if strings.Contains(seen, "NOTICE-GATED") {
				t.Fatalf("a refused notice still reached the PTY: %q", seen)
			}
		})
	}
}
