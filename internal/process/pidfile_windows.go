...
// On Windows, it uses os.TempDir()/agentfactory/<name>.pid.
...
func (p *PIDFile) Write(pid int) error {
	...
	if err := os.WriteFile(p.path, data, 0o600); err != nil { //nolint:gosec
...
// On Windows, stale detection is not performed — the recorded PID is returned as-is.
