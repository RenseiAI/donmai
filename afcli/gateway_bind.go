...
func bindWorkerGateway(ctx context.Context, logger *slog.Logger, d *daemon.SessionDetail, qw *runner.QueuedWork, harnessID string, sinkOverride ...costfeed.Sink) (*workerGateway, error) {
	...
	if admitted := qw.ResolvedProfile.Endpoint; admitted != nil {
		if admitted.Company != binding.Company || admitted.Model != binding.Model || admitted.Protocol != binding.Protocol || admitted.Host != binding.Host {
			stopCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			defer cancel()
			_ = g.Stop(stopCtx)
			return nil, errors.New("worker-local gateway binding does not match the admitted endpoint company, model, protocol, and host")
		}
		// Preserve the complete stable admission identity and add only the
		// worker-local delivery material generated by the gateway.
		merged := *admitted
		merged.BaseURL = binding.BaseURL
		merged.Env = binding.Env
		merged.Auth = binding.Auth
		merged.Region = binding.Region
		merged.CostModel = binding.CostModel
		merged.BringsOwnAuth = binding.BringsOwnAuth
		qw.ResolvedProfile.Endpoint = &merged
	} else {
